export default ({ IDL }) => {
  ...
  const DefaultInfoExt = IDL.Record({
    ...
    host: IDL.Opt(IDL.Variant({ url: IDL.Text, canister: IDL.Principal })),
  ...
  const ICNSResolver = IDL.Service({
    ...
    getCanister: IDL.Func([IDL.Text, IDL.Text], [IDL.Opt(IDL.Principal)], []),
    getExtensionLimit: IDL.Func([], [IDL.Nat], ['query']),
    getHost: IDL.Func(
      [IDL.Text],
      [IDL.Opt(IDL.Variant({ url: IDL.Text, canister: IDL.Principal }))],
      ['query']
    ),
