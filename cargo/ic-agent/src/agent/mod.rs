...
#[no_mangle]
pub fn ios_verify_json_example() -> *mut c_char {
    let  json = "{\"request_id\":[7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7],\"cert\":{\"tree\":[0],\"signature\":[],\"delegation\":null}}";
    return ios_verify_json(CString::new(json).unwrap().into_raw())
}


#[no_mangle]
pub fn ios_verify_json(to: *const c_char) -> *mut c_char {
    ...
    let IOSVerify {request_id, cert} =
        match serde_json::from_slice(&b[..]) {
            Ok(v) => v,
            Err(e) => return error("Deserialization", e.to_string()).into_raw(),
        };
    let json_response = match ios_verify(&request_id, cert) {
        Ok(response) => match serde_json::to_vec(&response) {
            Ok(v) => v,
            Err(e) => return error("Serialization", e.to_string()).into_raw(),
        },
        Err(e) => return error("Verification", e.to_string()).into_raw(),
    };
    CString::new(json_response).unwrap().into_raw()
}

#[no_mangle]
pub extern fn interchange_demo(to: *const c_char) -> *mut c_char {
    let c_str = unsafe { CStr::from_ptr(to) };
    ...
    CString::new("SIGNED ".to_owned() + recipient).unwrap().into_raw()
}
